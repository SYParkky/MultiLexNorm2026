...
# 후처리 파라미터
MFR_THRESHOLD   = 0.3   # train에서 이 비율 미만으로 변환됐으면 → 원본 유지
CONF_THRESHOLD  = 0.5   # 모델 confidence 이 미만이면 → 원본 유지
USE_MFR         = True
USE_CONF        = True
...
if __name__ == "__main__":
    ...
    for group, langs in GROUP_LANGS.items():
        ...
        # 결과 저장
        out_dir = os.path.join(MODELS_DIR, group, "postprocess_results")
    ...
    with open(os.path.join(MODELS_DIR, "postprocess_summary.json"), 'w', encoding='utf-8') as f:
